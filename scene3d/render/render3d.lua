...
function M.view_from_rotation(quat, viewport)
	...
	xmath.rotate(viewport.view_front, quat, M.FORWARD)
	xmath.rotate(viewport.view_right, quat, M.RIGHT)
	xmath.rotate(viewport.view_up, quat, M.UP)
end
...
local LOOK_AT_V = vmath.vector3()
function M.camera_view(viewport)
	if not viewport then
		xmath.add(LOOK_AT_V, M.view_position, M.view_front)
		return vmath.matrix4_look_at(M.view_position, LOOK_AT_V, M.view_up)
	else
		xmath.add(LOOK_AT_V, viewport.view_position, viewport.view_front)
		return vmath.matrix4_look_at(viewport.view_position, LOOK_AT_V, viewport.view_world_up)
	end
end

function M.camera_perspective(fov, aspect_ratio, near, far)
	return vmath.matrix4_perspective(fov or M.fov, aspect_ratio or M.aspect_ratio, near or M.near, far or M.far)
end
